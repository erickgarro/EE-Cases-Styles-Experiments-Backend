...
router.get('/get/reports', async (req, res) => {
  ...
  let header_kebab_vs_camel = 'user_id,age,gender,base_background,background,q_id,words,accu_k,accu_c,time_k,time_c\n';
  let header_mono_vs_color = 'user_id,age,gender,base_background,background,q_id,words,accu_km,accu_kc,accu_cm,accu_cc,time_km,time_kc,time_cm,time_cc\n';
  let header_case_color = 'user_id,age,gender,base_background,background,q_id,words,case,color,case_color,accu,time\n';

  let allResponses = [];
  let general = header_general;
  let kebab_vs_camel = header_kebab_vs_camel;
  let mono_vs_color = header_mono_vs_color;
  let case_color = header_case_color;
  ...
  for (let r in allResponses) {
    ...
    for (let a in answers) {
      ...
      let bkg = answer.background;

      if (answer.background !== 'n/a') {
        bkg = user.background.substring(0, 2);
      }

      kebab_vs_camel += `${user.userId},${user.age},${user.gender},${bkg},${user.background},${answer.parentQuestionId},${answer.words},${kebab_accu},${camel_accu},${kebab_median},${camel_median}\n`;

      mono_vs_color += `${user.userId},${user.age},${user.gender},${bkg},${user.background},${answer.parentQuestionId},${answer.words},${kebab_mono_accu},${kebab_color_accu},${camel_mono_accu},${camel_color_accu},${kebab_mono_median},${kebab_color_median},${camel_mono_median},${camel_color_median}\n`;

      case_color += `${user.userId},${user.age},${user.gender},${bkg},${user.background},${answer.parentQuestionId},${answer.words},${answer.case},${answer.color},${answer.caseColor},${answer.correct},${answer.time}\n`;
    }
  }
  ...
  zip.file(`case_color (${timestamp}).csv`, case_color);
